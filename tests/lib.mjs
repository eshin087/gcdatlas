...
export async function openPage({ width = 1280, height = 800, phone = false } = {}){
  ...
  await page.addInitScript(() => { window.__noAdapt = true; window.__syncCompile = true; try { localStorage.setItem('gcdatlas.dailySeen', JSON.stringify(new Date().toISOString().slice(0, 10))); } catch (e) {} });
