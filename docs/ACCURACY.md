...
| Sun glare from the planets | **Illustrative** | The Sun is drawn at its true size seen from each planet; the halo and spikes around it are a camera-style glare so a disc that small reads as blinding. |
| Betelgeuse surface | **Illustrative** | A few giant convection cells as in the ALMA and VLT images; the fine granules and filaments seen close up are simulated, not observed. |
