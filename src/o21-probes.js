...
// uP0.x: 0 = Voyager-style probe (dish, bus, booms), 1 = JWST (gold mirror over a five-layer sunshield); uP1: sun direction (world)
...
void main(){
  ...
  if(hit){
    ...
    vec3 base = m < 1.5 ? vec3(0.9, 0.9, 0.88) : (m < 2.5 ? vec3(0.55, 0.5, 0.42) : (m < 3.5 ? vec3(0.6, 0.6, 0.62) : (m < 4.5 ? vec3(0.75, 0.62, 0.85) : vec3(1., 0.78, 0.35))));
...
const newHorizons = addProbe({ key:'newhorizons', name:'New Horizons', label:'New Horizons', type:'space probe · first to Pluto', kind:0, sortKey:1.2,
  ...
  pos:V.mul(radecDir(hms(19,50), dms(-20,30)), 65*AU_LY), rad:0.006*KM, R0:frameY(V.mul(radecDir(hms(19,50), dms(-20,30)), -1), [0, 0, 1]), labelRange:0.01, distEarth:'~65 AU from the Sun',
  views:[{d:[0.6, 0.3, 0.75], k:2.4, hold:8, drift:0.05}], readout:() => '65 AU out · 14 km/s · powered by plutonium' });
