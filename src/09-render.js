...
let detailIdx = 1, glowOn = true, labelsOn = true;
...
function resize(){
  ...
  $('#btnDetail').textContent = 'detail ' + DETAIL[detailIdx].name;
}
...
function drawVolume(o, pr, rel, radius, setU, rot, vis = 1){
  ...
  gl.uniform1f(pr.u.uLod, clamp(rpx/(sceneH*0.3), 0.3, 1));
  setU && setU(pr);
  drawQuad();
  return true;
}
function drawParticles(o, s, vis = 1){
  ...
  const pr = P[s.prog]; gl.useProgram(pr.p);
...
function drawDrift(){
  ...
  for (const [pr, prim] of [[P.driftLn, gl.LINES], [P.driftPt, gl.POINTS]]){
    gl.useProgram(pr.p);
    gl.uniformMatrix3fv(pr.u.uCamRot, false, camRot); gl.uniform2f(pr.u.uTan, tanX, tanY);
    gl.uniform3fv(pr.u.uDrift, DR.off.map(x => x - Math.floor(x))); gl.uniform3fv(pr.u.uVel, M3.applyT(I3, DR.vel)); gl.uniform1f(pr.u.uZoom, clamp(DR.zoom, -0.4, 0.4));
...
function render(){
  ...
  for (const o of order){
    ...
    o.rpx = rpx;
    if (o.inRange && !o.inRange()){ o.vis = 0; continue; }
    const pmin = o.pxMin || 7;
    const vis = o.visFn ? o.visFn(rpx) : (o.alwaysFull ? 1 : smooth(pmin, pmin*2.6, rpx));
    o.vis = vis;
    if (vis > 0.003){
  ...
  gl.uniform1f(pr.u.uGlowAmt, glowOn ? 0.2 : 0); gl.uniform3f(pr.u.uBg, 0.012, 0.014, 0.026); gl.uniform4f(pr.u.uRect, -1, -1, 1, 1);
  drawQuad();
}
...
function goTo(i){ hideHint(); toggleAtlas(false); if (OBJ[i].marker) return; if (tour.on && TOUR.includes(i)){ tween = null; if (flight) finishFlightHere(); tourGo(i); } else lockOn(i); }
...
function updateLabels(){
  ...
  const ib = infoEl.getBoundingClientRect(), lb = ladderEl.getBoundingClientRect(), placed = [{ x:lb.left - 150, y:lb.top - 10, w:170, h:lb.height + 20 }];
  const cand = [];
  OBJ.forEach((o, i) => {
    const el = labelEls[i];
    let pr = null;
    ...
    if (labelsOn && !o.noLabel && !o.hidden && inScale && (!o.inRange || o.inRange())) pr = projectCSS(o.rel);
    if (pr){
  ...
  for (const c of cand){
    ...
    if (lx < ib.right + 8 && ly < ib.bottom + 4 && lx + w > ib.left) continue;
    if (placed.some(p => lx < p.x + p.w + 6 && lx + w + 6 > p.x && ly < p.y + (p.h || h) && ly + h > p.y)) continue;
...
// scale ladder: where the current view sits between a planet and the observable universe (camera distance)
const LADDER = [['Earth', 5e-9], ['Earth & Moon', 1e-7], ['Solar System', 6e-4], ['Oort cloud', 3], ['nearby stars', 80], ['Milky Way', 2.2e5], ['Local Group', 9e6], ['cosmic web', 2.5e9], ['observable universe', 1.3e11]];
const LAD_LO = -9.3, LAD_HI = 11.3;
const ladderEl = $('#ladder'), ladMark = $('#ladMark'), ladTxt = $('#ladTxt');
LADDER.forEach(([name, d]) => {
  const b = document.createElement('button'); b.className = 'tick'; b.textContent = name; b.setAttribute('aria-label', 'zoom to ' + name + ' scale');
  b.style.top = (100 - (Math.log10(d) - LAD_LO)/(LAD_HI - LAD_LO)*100).toFixed(2) + '%';
  b.addEventListener('click', () => { hideHint(); zoomTo(d); toast(name + ' scale · ' + fmtDist(d) + ' away'); });
  ladderEl.appendChild(b);
});
function updateLadder(){
  const eff = Math.max(orbit.dist, 1e-30)*kmPerLy()/LY;
  const f = clamp((Math.log10(eff) - LAD_LO)/(LAD_HI - LAD_LO), 0, 1);
  ladMark.style.top = ((1 - f)*100).toFixed(2) + '%';
  ladTxt.textContent = fmtDist(eff);
}
function cycleSpeed(){
  const S = [[1,'time 1x'],[3,'time 3x'],[0.25,'time 0.25x'],[0,'time paused']];
  const i = (S.findIndex(s => s[0] === timeScale) + 1) % S.length;
  timeScale = S[i][0]; $('#btnSpeed').textContent = S[i][1]; toast(S[i][1]);
}
function cycleDetail(){ detailIdx = (detailIdx + 1) % DETAIL.length; resize(); toast('detail: ' + DETAIL[detailIdx].name + ` (${cols} x ${rows} characters)`); }
function toggleGlow(){ glowOn = !glowOn; $('#btnGlow').setAttribute('aria-pressed', String(glowOn)); }
function toggleLabels(){ labelsOn = !labelsOn; $('#btnLabels').setAttribute('aria-pressed', String(labelsOn)); }
function toggleHelp(on){ $('#help').hidden = !on; if (on) $('#helpClose').focus(); else canvas.focus({preventScroll:true}); }
// ---------------------------------------------------------------- atlas picker
const atlasEl = $('#atlas'), atlasList = $('#atlasList'), atlasSearch = $('#atlasSearch');
...
GROUPS.forEach(([g, title]) => {
  ...
  items.forEach(o => {
    const b = document.createElement('button'); b.className = 'arow';
    ...
    atlasList.appendChild(b); atlasRows.push({ b, o, h });
  });
});
function atlasMark(i){ atlasRows.forEach(r => r.b.setAttribute('aria-current', String(r.o.index === i))); }
function toggleAtlas(on){
  atlasEl.hidden = !on; $('#btnAtlas').setAttribute('aria-expanded', String(on));
  if (on){ atlasSearch.value = ''; filterAtlas(''); const cur = atlasRows.find(r => r.o.index === infoObj); if (cur) cur.b.scrollIntoView({ block:'center' }); if (!IS_SMALL) setTimeout(() => atlasSearch.focus({preventScroll:true}), 30); }
}
function filterAtlas(q){
  q = q.trim().toLowerCase();
  atlasRows.forEach(r => { r.b.hidden = !!q && !(r.o.name + ' ' + (r.o.label || '') + ' ' + r.o.type + ' ' + (r.o.aka || '')).toLowerCase().includes(q); });
  atlasList.querySelectorAll('.agroup').forEach(h => { let n = h.nextElementSibling, any = false; while (n && !n.classList.contains('agroup')){ if (!n.hidden) any = true; n = n.nextElementSibling; } h.hidden = !any; });
}
atlasSearch.addEventListener('input', () => filterAtlas(atlasSearch.value));
atlasSearch.addEventListener('keydown', e => { if (e.key === 'Enter'){ const r = atlasRows.find(r => !r.b.hidden); if (r) goTo(r.o.index); } if (e.key === 'Escape') toggleAtlas(false); });
...
$('#btnDetail').addEventListener('click', cycleDetail);
...
// ================================================================ main loop
let last = performance.now(), ema = 16, adaptCount = 0, runTime = 0, resizePending = false, refocusT = 0;
addEventListener('resize', () => { if (resizePending) return; resizePending = true; requestAnimationFrame(() => { resizePending = false; resize(); }); });
...
function tick(dt){
  ...
  for (const o of OBJ){ o.t += sdt; o.update && o.update(sdt); if (o.parent && !o.selfPos) o.pos = V.add(o.parent.pos, o.offset); }
...
function frame(now){
  ...
  if (!window.__noAdapt && runTime > 4 && ema > 45 && adaptCount < 2 && detailIdx < DETAIL.length - 1 && document.visibilityState === 'visible'){
...
tick(0);
updateModeUI();
window.__cosmos = { dbg:{ imp, impSpec, get cols(){ return cols; }, get sceneH(){ return sceneH; } }, OBJ, BYKEY, tourGo, lockOn, setTour, cam, orbit, tour, TOUR, setDetail:i => { detailIdx = i; resize(); }, render, zoomTo, tick, flightDur:() => flight ? flight.dur : 0, hud:() => { roTimer = 0; updateHUD(0.2); },
